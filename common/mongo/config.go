...
func BindConfig(config *Config) {

	flag.StringVar(&config.URI, "mongo.uri", "", "the mongodb uri")
	flag.StringVar(&config.Database, "mongo.database", "", "the mongodb database")
	flag.IntVar(&config.Timeout, "mongo.timeout", DefaultTimeout, "the mongodb connection timeout in sec.")
	flag.BoolVar(&config.UseSSL, "mongo.useSSL", false, "use SSL with mongo")
	flag.StringVar(&config.SSLClientCertFile, "mongo.sslClientCertFile", "", "the mongodb sslClientCertFile")
	flag.StringVar(&config.SSLClientKeyFile, "mongo.sslClientKeyFile", "", "the mongodb sslClientKeyFile")
}
